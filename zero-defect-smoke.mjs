...
await scenario("catalog-desktop", { width: 1440, height: 1000 }, async (page) => {
  ...
  return { status: response?.status(), initialCards, filteredCards, preciseFilters, serviceFacts: await page.locator('.service-facts-bar').count() === 1, onePrimaryHeroAction: await page.locator('.service-hero .primary-government-cta:visible').count() === 1, directServiceUrl: page.url().includes("/services/") && !page.url().includes("?q="), hasOfficialCta: await page.locator('.official-source-panel a[href^="https://"]').count() >= 1, routeMode: await page.locator(".service-detail").getAttribute("data-official-route-mode"), firstHref };
});
