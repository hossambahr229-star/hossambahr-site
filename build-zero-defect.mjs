...
const services = approved.map((record, index) => {
  ...
  return {
    ...
    loginRequired: /login|account|smartservices|echannels|eservices/i.test(record.officialUrl),
  };
});
...
function servicePage(service) {
  ...
  const schema = { "@context": "https://schema.org", "@type": "GovernmentService", name: service.name, serviceType: service.officialName, areaServed: service.emirate, provider: { "@type": "GovernmentOrganization", name: service.authority.name }, url: `https://hossambahr.com${service.internalUrl}`, sameAs: service.officialUrl };
  return document({
    title: service.name,
    description: service.description,
    canonical: service.internalUrl,
    schema,
    body: `<main id="main-content" class="page-shell service-detail"><nav class="breadcrumbs" aria-label="مسار الصفحة"><a href="/">الرئيسية</a><span>←</span><a href="/services/">الخدمات</a><span>←</span><a href="/categories/${service.category}/">${escapeHtml(category.title)}</a></nav><header class="page-hero service-hero"><span class="eyebrow">${escapeHtml(service.authority.name)} · ${escapeHtml(service.emirate)}</span><h1>${escapeHtml(service.name)}</h1><p class="official-name">الاسم الرسمي: ${escapeHtml(service.officialName)}</p><p>${escapeHtml(service.description)}</p><div class="actions"><a href="${escapeHtml(service.officialUrl)}" rel="noopener noreferrer">ابدأ من القناة الرسمية ↗</a><a class="secondary" href="#requirements">راجع المتطلبات</a></div><p class="verification-note">تم التحقق من تطابق الرابط مع الخدمة بتاريخ ${escapeHtml(service.lastReviewed)}. ${service.loginRequired ? "قد تطلب البوابة تسجيل الدخول قبل متابعة الطلب." : "يفتح الرابط صفحة الخدمة أو مسارها الرسمي."}</p></header><div class="detail-grid"><section id="requirements" class="content-panel"><h2>المتطلبات والمستندات</h2><ul>${requirements}</ul></section><section class="content-panel"><h2>الرسوم</h2><p>${escapeHtml(service.fees)}</p></section><section class="content-panel"><h2>المدة المتوقعة</h2><p>${escapeHtml(service.duration)}</p></section><section class="content-panel"><h2>الشروط</h2><p>${escapeHtml(service.conditions || "تحدد الجهة الرسمية الأهلية بحسب بيانات الطلب.")}</p></section><section class="content-panel"><h2>الحالات الخاصة وأسباب التعطل</h2><p>${escapeHtml(service.specialCases || "تحقق من تطابق البيانات والمستندات قبل الإرسال.")}</p></section><section class="content-panel"><h2>بيانات الخدمة</h2><dl class="service-facts"><dt>نوع الطلب</dt><dd>${escapeHtml(service.type)}</dd><dt>الإمارة</dt><dd>${escapeHtml(service.emirate)}</dd><dt>الجهة</dt><dd><a href="/authorities/${service.authority.slug}/">${escapeHtml(service.authority.name)}</a></dd><dt>الحالة</dt><dd>موثقة</dd></dl></section></div><section class="content-section"><h2>الأسئلة الشائعة</h2><div class="faq-list">${faq}</div></section><section class="content-section"><h2>خدمات مرتبطة من الجهة نفسها</h2><ul class="related-list">${related}</ul><div class="sequence-actions">${service.previousService ? `<a href="${service.previousService}">الخدمة السابقة</a>` : "<span></span>"}${service.nextService ? `<a href="${service.nextService}">الخدمة التالية</a>` : ""}</div></section><section class="official-source-panel"><h2>المصدر والتنفيذ الرسمي</h2><p>لا تستبدل هذه الصفحة القناة الحكومية. راجع المصدر قبل السداد أو رفع المستندات.</p><a href="${escapeHtml(service.officialUrl)}" rel="noopener noreferrer">فتح الخدمة الرسمية ↗</a></section></main>`,
  });
}
