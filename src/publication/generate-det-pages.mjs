...
const normalizedHistoryLinks = publication.services.filter((record) => record.normalization?.excludeFromRealTotal).map((record) => {
  const service = sourceFor(record);
  return `<li><a href="/services/${esc(record.slug)}/">${esc(service.platformTitle)}</a> — ${esc(record.normalization.resolution)}</li>`;
}).join('');
const verifiedCards = matrix.services.map((service) => `<article class="card" data-directory-card data-search="${esc([service.name, service.officialName, service.authority.name, service.emirate, service.category, service.type].join(' ').toLowerCase())}"><div class="card-meta"><span>${esc(service.authority.name)}</span><span>${normalizedGdrfaIds.has(service.id) ? 'خدمة فرعية مطبّعة' : 'متحقق'}</span></div><h3><a href="${esc(service.internalUrl)}">${esc(service.name)}</a></h3><p>${esc(service.description)}</p><div class="actions"><a href="${esc(service.internalUrl)}">عرض التفاصيل</a></div></article>`).join('');
...
const directory = shell('دليل الخدمات', `<main id="main-content" class="page-shell"><nav class="breadcrumbs"><a href="/">الرئيسية</a><span>←</span><span>الخدمات</span></nav><header class="page-hero"><span class="eyebrow">دليل الخدمات الآمن</span><h1>الخدمات الحكومية</h1><p>يضم الدليل الخدمات المتحققة، إضافة إلى خدمات DET قيد التحقق بصفحات داخلية آمنة وأزرار حكومية معطّلة.</p></header><form class="search-shell" onsubmit="return false"><div class="search-row"><input id="det-search" aria-label="ابحث في الخدمات" placeholder="الاسم، الجهة، الإمارة، النشاط، نوع الرخصة أو رقم التصنيف"><button id="det-search-button" type="button">بحث</button></div></form><section class="content-section"><div id="det-results" class="cards">${verifiedCards}${canonicalCards}${detCards}</div><p id="det-empty" hidden>لا توجد نتيجة مطابقة.</p></section><section class="detail-section" aria-labelledby="normalized-history-title"><h2 id="normalized-history-title">سجلات تاريخية مطبّعة</h2><p>حُفظت هذه المسارات للتوافق مع الروابط القديمة فقط، وهي تشرح المعاملات الفعلية التي حلت محل التسميات المركبة دون عرض CTA حكومي مستقل.</p><ul>${normalizedHistoryLinks}</ul></section><script>const input=document.querySelector('#det-search'),cards=[...document.querySelectorAll('[data-directory-card]')],empty=document.querySelector('#det-empty');function normalize(value){return value.trim().toLowerCase().replace(/[أإآ]/g,'ا').replace(/ة/g,'ه').replace(/ى/g,'ي').replace(/\s+/g,' ')}function run(){const terms=normalize(input.value).split(' ').filter(Boolean);let shown=0;cards.forEach(c=>{const haystack=normalize(c.dataset.search);const ok=terms.length===0||terms.every(term=>haystack.includes(term));c.hidden=!ok;if(ok)shown++});empty.hidden=shown!==0}document.querySelector('#det-search-button').addEventListener('click',run);input.addEventListener('input',run);const q=new URLSearchParams(location.search).get('q');if(q){input.value=q;run()}</script></main>`);
