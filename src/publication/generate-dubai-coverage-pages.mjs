...
const audit = JSON.parse(await readFile(resolve(root, 'content/dubai-coverage-expansion.json'), 'utf8'));
...
for (const authority of audit.authorities) {
  ...
  const cards = all.map((service) => `<article class="card" data-directory-card data-search="${esc([service.nameAr, service.nameEn, service.category, authority.labelAr, authority.labelEn, 'دبي'].join(' ').toLowerCase())}"><div class="card-meta"><span>${esc(authority.labelEn)}</span><span>متحقق</span></div><h2><a href="/services/${esc(service.slug)}/">${esc(service.nameAr)}</a></h2><p>${esc(service.description)}</p><div class="actions"><a href="/services/${esc(service.slug)}/">عرض التفاصيل</a></div></article>`).join('');
  ...
  const additions = authority.newVerifiedServices.map((service) => `<article class="card dubai-coverage-added" data-directory-card data-search="${esc([service.nameAr, service.nameEn, service.category, service.type, authority.labelAr, authority.labelEn, 'دبي'].join(' ').toLowerCase())}"><div class="card-meta"><span>${esc(authority.labelEn)}</span><span>متحقق</span></div><h3><a href="/services/${esc(service.slug)}/">${esc(service.nameAr)}</a></h3><p>${esc(service.description)}</p><div class="actions"><a href="/services/${esc(service.slug)}/">عرض التفاصيل</a></div></article>`).join('');
