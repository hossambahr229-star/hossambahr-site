...
function page(authority, service) {
  const guidance = service.destinationKind === 'OFFICIAL_GUIDANCE';
  const destinationLabel = guidance ? 'دليل حكومي رسمي متحقق' : 'بطاقة خدمة حكومية متحققة';
  const destinationExplanation = guidance
    ? `لا تنشر الجهة رابط تنفيذ عميقًا ثابتًا لهذه المعاملة. يفتح الرابط دليلًا حكوميًا يذكر المعاملة بالاسم ويوجه إلى قناة التنفيذ الصحيحة لدى ${authority.labelAr}.`
    : `الرابط يفتح بطاقة المعاملة المحددة لدى ${authority.labelAr}، وليس صفحة رئيسية أو خدمة مشابهة.`;
  const ctaLabel = guidance ? 'فتح الدليل الحكومي الرسمي وطريقة الوصول' : 'فتح بطاقة الخدمة الحكومية الرسمية';
  return shell(service.nameAr, service.description, `<main id="main-content" class="page-shell" data-publication-state="VERIFIED" data-destination-kind="${guidance ? 'OFFICIAL_GUIDANCE' : 'DIRECT_SERVICE'}" data-government-authority="${esc(authority.id)}"><nav class="breadcrumbs"><a href="/">الرئيسية</a><span>←</span><a href="/authorities/${esc(authority.id)}/">${esc(authority.labelEn)}</a><span>←</span><span>${esc(service.nameAr)}</span></nav><header class="page-hero"><span class="eyebrow">${esc(authority.labelAr)}</span><h1>${esc(service.nameAr)}</h1><p>${esc(service.description)}</p><p class="official-name">الاسم الرسمي: ${esc(service.nameEn)}</p></header><div class="scope-note"><b>${destinationLabel}:</b> ${esc(destinationExplanation)}</div><div class="detail-layout"><div class="detail-content"><section class="detail-section"><h2>المستندات والمتطلبات</h2>${list(service.requirements)}</section><section class="detail-section"><h2>الشروط</h2><p>${esc(service.conditions)}</p></section><section class="detail-section"><h2>الرسوم الرسمية المنشورة</h2><p>${esc(service.fees)}</p></section><section class="detail-section"><h2>مدة الإنجاز</h2><p>${esc(service.duration)}</p></section><section class="detail-section"><h2>خطوات التنفيذ</h2><ol><li>فتح ${guidance ? 'الدليل الحكومي الرسمي' : 'بطاقة الخدمة الرسمية'}.</li><li>${guidance ? 'اختيار المعاملة بالاسم ونوع الطلب المطابق.' : 'اختيار القناة المتاحة وتسجيل الدخول عند الحاجة.'}</li><li>إدخال البيانات وإرفاق المستندات.</li><li>سداد الرسوم واستلام المخرج الرسمي.</li></ol></section><section class="detail-section"><h2>الأسئلة الشائعة</h2><details><summary>هل الرابط خاص بهذه المعاملة؟</summary><p>${guidance ? 'الرابط هو أقرب مسار حكومي موثق ويعرض المعاملة ضمن دليل الجهة؛ لم يتم اختراع رابط تنفيذ عميق غير منشور.' : 'نعم، تمت مطابقة عنوان الرابط ومحتواه مع المعاملة المحددة.'}</p></details><details><summary>هل يمكن أن تتغير الرسوم؟</summary><p>تعرض الجهة الحكومية القيمة النهائية قبل السداد، وتختلف بعض الرسوم بحسب النوع أو القناة.</p></details></section></div><aside class="service-aside"><span class="status-good">${destinationLabel}</span><dl><dt>الإمارة</dt><dd>${esc(authority.emirate)}</dd><dt>الجهة</dt><dd>${esc(authority.labelAr)}</dd><dt>الفئة</dt><dd>${esc(service.category)}</dd><dt>نوع الطلب</dt><dd>${esc(service.type)}</dd><dt>نوع الوجهة</dt><dd>${guidance ? 'دليل رسمي' : 'خدمة مباشرة'}</dd></dl><div class="actions"><a data-government-cta="verified" data-destination-kind="${guidance ? 'official-guidance' : 'direct-service'}" href="${esc(service.officialUrl)}" rel="noopener noreferrer">${ctaLabel}</a><a class="secondary" href="/authorities/${esc(authority.id)}/">العودة إلى خدمات الجهة</a></div></aside></div></main>`);
}
