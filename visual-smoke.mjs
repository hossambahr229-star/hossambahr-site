...
if (siteRoot) {
  ...
  server = createServer(async (request, response) => {
    try {
      ...
      const routedFile = requested.endsWith("/") ? `${requested}index.html` : requested;
      const file = resolve(siteRoot, routedFile);
...
const summary = {
  ...
  pagesWithHeritageIdentity: results.filter((result) => result.hasHeritageIdentity).length,
